...
class Graph:
    ...
    def _reset_interDC_matrix(self):
        self._interDC_matrix = [[[0, []] for _ in range(self.num_DC)] for _ in range(self.num_DC)]
    ...
    async def pagerank(self, inter_DC=True, num_iterations: int = 100, d: float = 0.85):
        if not inter_DC:
            return await self._pgr(num_iterations, d)
        else:
            return await self._pgrdc(num_iterations, d)
    ...
    async def _build_rank(self,d):
        ...
        # send in parallel        
        await asyncio.gather(*(self._send_interDC_data(targetDC=dc) for dc in range(self.num_DC)))
    ...
    async def _pgrdc(self, num_iterations, d) -> np.array:
